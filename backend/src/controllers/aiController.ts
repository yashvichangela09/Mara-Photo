...
// 0.45 is the clean high-precision threshold: returns 100% accurate results without extra/wrong photos
 ...
 * where 0.0 distance -> 100% similarity, and 0.45 distance -> 80% similarity.
 */
const distanceToSimilarity = (dist: number): number => {
  if (dist <= 0.45) {
    return 1.0 - (dist / 0.45) * 0.20;
  }
  return 0;
};
