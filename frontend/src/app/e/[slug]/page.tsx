...
export default function ClientGallery() {
    ...
    try {
      ...
      for (let i = 0; i < downloads.length; i++) {
        ...
        try {
          ...
          console.error("Failed to fetch photo for zip", fetchErr);
        }
      }
      ...
      const a = document.createElement('a');
      a.href = blobUrl;
      a.download = `${event?.name || 'Photos'}_selected.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(blobUrl);
  ...
  if (isLocked) {
    return (
      <div className="min-h-screen bg-[#faf9f6] text-[#09090b] flex flex-col items-center justify-center p-6 relative overflow-hidden font-poppins">
        ...
        <div className="w-full max-w-md bg-white/90 backdrop-blur-xl border border-[#e3d8c8]/40 p-8 sm:p-10 rounded-3xl text-center shadow-2xl relative z-10">
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto justify-end">
          ...
          {isMultiSelect ? (
            <div className="flex items-center gap-3">
              ...
              <button 
                onClick={() => {
                  const allIds = galleryMedia.map(m => m._id);
                ...
                className="text-xs text-[#c5a880] hover:underline font-bold transition-all px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-[#faf9f6] border border-slate-200"
              >
                {selectedMediaIds.length === galleryMedia.length ? 'Deselect All' : 'Select All'}
              </button>
              <button 
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pb-4">
              {galleryMedia.map((m, idx) => {
                ...
                return (
                  <div 
                    key={m._id || idx}
                    className={`relative aspect-square rounded-2xl overflow-hidden bg-slate-100 border transition-all duration-300 group cursor-pointer ${
                      isSelected 
                        ? 'border-2 border-[#FF6B00] ring-4 ring-[#FF6B00]/20 shadow-lg scale-95' 
                        : 'border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-[#c5a880]'
                    }`}
                  >
                    {/* Media content */}
                    {m.type === 'VIDEO' ? (
                      <div className="w-full h-full relative bg-[#09090b]">
                        <video 
                          ...
                          preload="metadata"
                        />
                        <div className="absolute inset-0 flex items-center justify-center bg-black/10 group-hover:bg-black/25 transition-colors pointer-events-none z-10">
                    ...
                    {searchActive && m.similarityPercent && (
                      <div className="absolute top-2.5 left-2.5 z-20">
                        <div className={`px-2 py-0.5 rounded-lg text-[9px] font-bold backdrop-blur-md border flex items-center gap-1 ${
                          m.confidence === 'HIGH' 
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          ...
          <div className="flex flex-wrap items-center gap-6 text-xs text-slate-300 font-medium">
            {event?.studioId?.instagramUrl && (
              <a href={event.studioId.instagramUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2.5 rounded-xl transition-all duration-300 hover:border-[#c5a880]/30 hover:scale-[1.03]">
            ...
            {event?.studioId?.facebookUrl && (
              <a href={event.studioId.facebookUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2.5 rounded-xl transition-all duration-300 hover:border-[#c5a880]/30 hover:scale-[1.03]">
                <svg className="w-4 h-4 fill-none stroke-current text-[#c5a880]" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
            ...
            <div className="relative inline-block max-w-[85vw] max-h-[85vh] rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="absolute top-4 right-4 z-30 flex items-center gap-2 bg-black/40 backdrop-blur-md p-1.5 rounded-full border border-white/20 shadow-xl pointer-events-auto">
                ...
                <button 
                  ...
                  className="p-2.5 rounded-full bg-[#c5a880] hover:bg-[#b59a72] text-[#09090b] transition-all cursor-pointer shadow-md hover:scale-105"
